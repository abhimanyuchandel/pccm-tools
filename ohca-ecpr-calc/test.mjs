...
assert.match(html, /<title>Out-of-hospital ECPR favorable CPC calculator<\/title>/);
assert.match(html, /<h1>Out-of-hospital ECPR favorable CPC calculator<\/h1>/);
assert.equal((html.match(/Last updated:/g) || []).length, 1);
const about = /<details class="about">([\s\S]*?)<\/details>/.exec(html)?.[1];
assert.ok(about);
assert.match(about, /<p class="updated">Last updated: <time datetime="2026-09-21">September 21, 2026<\/time><\/p>\s*<\/div>\s*$/);

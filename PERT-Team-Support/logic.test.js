...
test("PERT page no longer exposes the audited contradictory strings", () => {
  ...
  assert.ok(html.includes("Advanced therapy: this profile may be appropriate for early catheter-based reperfusion discussion (including HI-PEITHO-type candidates) if local expertise is available and bleeding risk is acceptable."));
  ...
  assert.ok(html.includes("Risk stratification: obtain/review TTE if RV status is unknown, discordant, or the patient is clinically worsening."));
  assert.ok(html.includes("Suspected PE with delayed imaging: if clinical suspicion is high and no absolute anticoagulation contraindication is present, start empiric therapeutic anticoagulation while expediting definitive imaging."));
  assert.ok(html.includes("Anticoagulation: start therapeutic anticoagulation now. See Medication Recommendations for agent, dose, and route."));
  assert.ok(html.includes("Anticoagulation: therapeutic anticoagulation is currently contraindicated. Reassess daily; if protection is needed before anticoagulation is feasible, consider a retrievable IVC filter based on clot burden and bleeding risk."));
  assert.ok(html.includes("Advanced therapy: activate reperfusion planning now with PERT/IR/cardiology/CT surgery and use the fastest locally available appropriate modality."));
  assert.ok(html.includes("Advanced therapy: pregnancy is a relative bleeding consideration, not an absolute barrier to reperfusion. If escalation becomes necessary, obtain immediate multidisciplinary review and use the fastest locally appropriate reperfusion strategy."));
  assert.ok(html.includes("If an endovascular PE procedure is likely, choose UFH vs LMWH based on hemodynamic stability, expected need for urgent reversal/procedure, renal function, HIT history, and local protocol."));
  assert.ok(html.includes("Obtain baseline CBC, hemoglobin, platelets, and creatinine/eGFR. Then monitor anticoagulant-specific labs only: UFH/argatroban -> aPTT or anti-Xa per protocol; warfarin bridge -> INR; LMWH/DOAC -> no routine PT/INR/aPTT."));
  assert.ok(html.includes("LMWH/DOAC pathway: do not order routine PT/INR/aPTT to monitor effect. Follow CBC/hemoglobin, creatinine/eGFR, weight change when clinically relevant, drug interactions, and bleeding signs."));
  assert.ok(html.includes('recommendations.push("[COR 2a] In suspected acute PE with an AHA/ACC category C2 or higher profile, if clinical suspicion is high and no absolute anticoagulation contraindication is present, therapeutic anticoagulation is reasonable while definitive imaging is being expedited.");'));
  ...
  assert.ok(!html.includes("Risk stratification: consider transthoracic echocardiography for further risk stratification if not already performed."));
  assert.ok(!html.includes("Suspected PE: in suspected acute PE with an AHA/ACC category C2 or higher profile and low bleeding risk, consider administration of therapeutic anticoagulation when imaging is delayed or not immediately accessible."));
  assert.ok(!html.includes("Anticoagulation: oral option apixaban 10 mg PO twice daily for 7 days, then 5 mg PO twice daily; parenteral option enoxaparin 1 mg/kg SC every 12 hours."));
  assert.ok(!html.includes("Anticoagulation: therapeutic anticoagulation is contraindicated; defer anticoagulation and use a non-anticoagulation pathway."));
  assert.ok(!html.includes("Advanced therapy: consider systemic thrombolysis (if bleeding risk acceptable), CDL, MT, or surgical thrombectomy (based on local capabilities and specialty consultation)."));
  assert.ok(!html.includes("If the patient is likely to undergo an endovascular PE procedure, favor LMWH (enoxaparin 1 mg/kg SC every 12 hours) over UFH to provide more effective anticoagulation."));
  assert.ok(!html.includes("Check baseline and serial CBC, hemoglobin, platelets, creatinine/eGFR, PT/INR, and aPTT during acute therapy."));
  assert.ok(!html.includes("LMWH/DOAC pathway: routine coagulation labs are not required; monitor renal function, body weight change, interacting medications, and bleeding signs."));
  ...
  assert.ok(!html.includes('recommendations.push("[Operational] Avoid deep sedation and mechanical ventilation if possible; if they are required, optimize preload, vasoactive support, and RV afterload to reduce decompensation risk.");'));
